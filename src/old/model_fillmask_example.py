...
def collect_mlm_data(
    ...
    existing_subjects = {
        'train': {rel: None for rel in RELATIONS},
    ...
    for subset in [
        ...
        with open(f'./data/{subset}.jsonl', 'r', encoding='utf8') as rfp:
            for lidx, line in enumerate(rfp):
                ...
                if subset in ['train', 'dev', 'test']:
                    ...
                    existing_subjects[subset][entry['Relation']].append(
                        entry['SubjectEntity']
                    )
                else:
                    ...
                    for ref_subset in existing_subjects:
                        ...
                        if (
                            entry['SubjectEntity']
                            in existing_subjects[ref_subset][entry['Relation']]
                        ):
                ...
                _, final_prompts = perentry_prompting(
                    entry, pipe, tokenizer.mask_token, key_priors, prompt_creator
                )

                curr_positives = (
                    entry['ObjectEntities'] if 'ObjectEntities' in entry else []
                )
        ...
        print(
            f"subset {subset}; subj_already_exist_cnt: {subj_already_exist_cnt} / {total_cnt};"
        )
                ...
                for line in rfp:
                    ...
                    for mt in messy_temps:  # manually disable the messy mined templates
                        if mt in item['input_ids']:
                            continue
                    if (
                        concentrated_rels is not None
                        and item['Relation'] not in concentrated_rels
                    ):
                        continue
                    posi_labels_set = (
                        random.sample(item['posi_labels'], k=5)
                        if len(item['posi_labels']) > 5
                        else item['posi_labels']
                    )
                    for posi_obj in posi_labels_set:
                        ...
                        mask_word_end = min(
                            mask_word_end + extend_len, len(instantiated)
                        )

                        instantiated = self.tokenizer(
                            instantiated, add_special_tokens=True, padding=True
                        )
                        instantiated_wordids = instantiated.word_ids()
                        instantiated = dict(instantiated)
                        for tidx in range(len(instantiated['input_ids'])):
                            if (
                                instantiated_wordids[tidx] is not None
                                and mask_word_start
                                <= instantiated_wordids[tidx]
                                < mask_word_end
                            ):
                                masked_instantiated = copy.deepcopy(
                                    instantiated['input_ids']
                                )
                                masked_instantiated[tidx] = self.tokenizer.mask_token_id
                                out_item = {}
                                out_item['labels'] = [
                                    x if i == tidx else -100
                                    for i, x in enumerate(instantiated['input_ids'])
                                ]
                                out_item['input_ids'] = masked_instantiated
                                out_item['attention_mask'] = copy.deepcopy(
                                    instantiated['attention_mask']
                                )
...
def train(
    ...
    bert_collator = transformers.DataCollatorForTokenClassification(
        tokenizer=bert_tokenizer, padding=True, max_length=128
    )
...
def inference(inference_fn: str, ckpt_dir: str, mode: str):
    ...
    if mode == 'eval':
        inference_dataset = MLMDataset(
            inference_fn, tokenizer=bert_tokenizer, extend_len=0
        )
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--ckpt_dir', type=str, default='../lmkbc_checkpoints/mlm_checkpoints%s'
    )
    ...
    if args.job_name == 'collect_data':
        ...
        with open(thresholds_fn, 'r', encoding='utf8') as tfp:
            ...
            prompts_per_rel = {
                rel: [tuple(x[2]) for x in prompt_order[rel][: num_prompts[rel]]]
        ...
        collect_mlm_data(
            args.model_name,
            top_k=args.top_k,
            gpu_id=args.collect_data_gpu_id,
            suffix=args.data_suffix,
            use_softmax=args.use_softmax,
            prompts_per_rel=prompts_per_rel,
        )
        ...
        if args.data_mode == 'development':
            ...
            train_fns = [
                os.path.join(
                    args.data_dir,
                    f'silver_train{args.silver_data_size}_mlm{args.data_suffix}.jsonl',
                )
            ]
            dev_fn = os.path.join(args.data_dir, f'train_mlm{args.data_suffix}.jsonl')
            test_fn = os.path.join(args.data_dir, f"dev_mlm{args.data_suffix}.jsonl")
        elif (
            args.data_mode == 'silver_B'
        ):  # tests for using silver and real training data jointly, with dev set as test
            train_fns = [
                os.path.join(
                    args.data_dir,
                    f'silver_train{args.silver_data_size}_mlm{args.data_suffix}.jsonl',
            ...
            args.data_mode == 'silver_C'
        ):  # using silver and real training data jointly, for submission
            train_fns = [
                os.path.join(
                    args.data_dir,
                    f'silver_train{args.silver_data_size}_mlm{args.data_suffix}.jsonl',
