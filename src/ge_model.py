...
def train():
    ...
    # )
    if args.recode_type != "null":
        print("recode token embedding")
        model = util.opt_token_layer(model,
        ...
        # model.resize_token_embeddings(len(enhance_tokenizer))
...
if __name__ == "__main__":
    ...
    origin_tokenizer = transformers.AutoTokenizer.from_pretrained(
        pretrained_model_name_or_path=args.model_load_dir,
        padding_side='left',
        padding=True,
    )
    if os.path.exists(config.OPT_TOKENIZER_PATH):
        enhance_tokenizer = transformers.AutoTokenizer.from_pretrained(
            pretrained_model_name_or_path=config.OPT_TOKENIZER_PATH,
            padding_side='left',
            padding=True,
        )
    else:
        ...
        with open(config.TOKENIZER_PATH+"/added_tokens.json") as f:
            ...
            for key, value in additional_token_dict.items():
                enhance_tokenizer.add_tokens(key)
        enhance_tokenizer.save_pretrained(config.OPT_TOKENIZER_PATH)
    ...
    if args.do_valid:
        test_pipeline()
        scores_per_relation = evaluate(args.output, args.test_fn)
        print(scores_per_relation.to_string(max_cols=12))
