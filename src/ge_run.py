...
task = "text-generation-token-recode"
print(torch.cuda.is_available())
...
def run():
   ...
   python src/ge_model.py  --test_fn {config.TRAIN_FN } --template_fn res/question-prompts0.csv  --output {OUTPUT_FILE} --train_fn data/train.jsonl --pretrain_model_name facebook/opt-350m  --train_batch_size 16 --gpu 0    --dev_fn  data/train_tiny.jsonl --do_train 1 --do_valid 1 --bin_dir best_ckpt  --train_epoch 20 --test_batch_size 32 --learning_rate 5e-4 --few_shot 3 --model_load_dir {model_load_dir} --model_save_dir {model_save_dir} --model_best_dir  {model_best_dir} --recode_type std
